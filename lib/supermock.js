 ...
 * @constructor
 */
var SuperMock = function SuperMock(args){
  args = args || {};
...
SuperMock.prototype.assertCalledWith = function(){
  ...
  for (var i=0;i<calledWith.length;i++) {
     try {
        assert.deepEqual(calledWith[i], args);
        return;
     } catch (err) {}
  }
  assert.deepEqual(calledWith,args,"was not called with " + args)
};
